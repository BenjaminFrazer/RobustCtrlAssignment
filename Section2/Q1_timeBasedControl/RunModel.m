...
Controler = "None";
saveFigPath = "/Figures/Section2/Q2";

%% Simulation setup
% Time
simulationTime_total = 8;           % in seconds *------* YOU CAN CHANGE THIS
...
for timeStep = 1:timeSteps_total
    ...
    if doTurnRight %test if we have reached angle
        if state(timeStep,stateEnum.angHeading)<=turnRightAngleFinal
            ...
            voltage_right = 6;
            voltage_left = 6;
            doTurnLeft= true; % we now allow the left turn to procede
        end
    end
...
% *----------------------------------*
if doTurnLeft || doTurnRight
    fileTag_turn = sprintf("_Turn(L%u,t%u_R%u,t%u)",doTurnLeft,round(turnLeftTime),doTurnLeft,round(turnRightTime));
else
    fileTag_turn = "";
end

FigTag="xVy";
thisFileName = fullfile(pwd,saveFigPath, strcat(FigTag,fileTag_turn,sprintf("_Vinit(l%d_Vr%d)",voltage_left_init,voltage_right_init)));
