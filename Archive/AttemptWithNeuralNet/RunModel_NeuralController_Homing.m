...
%% Simulation setup
% Time
simulationTime_total = 120;           % in seconds *------* YOU CAN CHANGE THIS
...
for timeStep = 1:timeSteps_total
    ...
    NeuralInput.Theta = wrapToPi((newHeadingAngle)-(state(timeStep,stateEnum.angHeading)));
%     NeuralInput.Theta = 0;
    if NeuralInput.LS <1 ||NeuralInput.RS <1 %engagte Collision avoidance
    NeuralInput.Theta = 0;
    end
% NeuralInput.Theta = 0;
...
%     NeuralInput.LS = 1;
%     NeuralInput.RS = 1;
    [out] = NeuralController_2(NeuralInput,Params);

    voltage_left = (out.ML)*4-2;
    voltage_right = (out.MR)*4-2;
